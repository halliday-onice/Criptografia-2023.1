...
void genBiasTable() {
    int bias[SBOXLEN][SBOXLEN]; //gonna save the bias information here
    ...
    int alpha = 0;
    for( ; counter < SBOXLEN; counter++ ) {
        printf("c|");
    }
    printf("\n & ");
	counter = 0;
    for( uchar x = 1; counter < SBOXLEN; x++, counter++ ) {
        if( x != SBOXLEN - 1 ) {
            printf(" {\\bf %.1X} &", x);
        } else {
            printf(" {\\bf %.1X} \\\\\\hline\n", x);
    ...
    for( uchar x = 1; counter < SBOXLEN; x++, counter++ ) {
        ...
        printf(" {\\bf %.1X} &", x);
		counter2 = 0;
        for( uchar beta = 1; counter2 < SBOXLEN; beta++, counter2++) {
            //printf("beta: %d", beta);
            alpha = 0;
			counter3 = 0;
            for( uchar X = 0; counter3 < SBOXLEN; X++, counter3++) {
				//printf("X: %d", X);
                int a;
